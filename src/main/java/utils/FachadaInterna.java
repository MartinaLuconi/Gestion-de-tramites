    ...
    public static FachadaInterna getInstance() {
     ...
     List<Object> buscar(String claseABuscar, List<DTOCriterio> criterioList) {
        ...
        if (criterioList != null) {
            ...
            // Si el atributo contiene un ".", es porque estamos buscando en una relación
            if (atributo.contains(".")) {
                String[] partes = atributo.split("\\.");
                String alias = partes[0];  // La primera parte es la entidad relacionada
                String subAtributo = partes[1];  // La segunda parte es el atributo de la entidad relacionada
                
                // Crear un alias para la entidad relacionada
                cr.createAlias(claseABuscar.toLowerCase() + "." + alias, alias);
                atributo = alias + "." + subAtributo;  // Usar el alias para la consulta
            }
